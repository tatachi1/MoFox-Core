...
**本次更新摘要**
- 对接 MoFox 六层数据库架构（API/优化/配置层），避免直接使用 `Session`
- 引入统一调度器用于记忆整理的后台批处理（定时/事件触发）
- 衔接事件系统与权限系统（Master 用户、权限节点）以保障安全与可控性
- 增补双轨记忆与兴趣系统的协同策略与检索增强建议
- 明确性能优化与缓存策略（L1/L2/L3）与批量操作最佳实践
...
### 2.1.1 系统集成视图（与 MoFox 主系统）
```
┌─────────────────────────────────────────────────────┐
│                     主系统 (MainSystem)             │
│  ┌────────────────────────┐   ┌───────────────────┐ │
│  │ 事件系统 EventManager  │   │ 统一调度器 Scheduler│ │
│  └────────────────────────┘   └───────────────────┘ │
│            ▲                         ▲              │
│            │ ON_MESSAGE/ON_START     │ TIME/EVENT   │
│            │                         │              │
│  ┌────────────────────────────────────────────────┐ │
│  │               记忆图模块 Memory Graph           │ │
│  │  提取/构建/检索/整理  + LLM 工具调用层          │ │
│  └────────────────────────────────────────────────┘ │
│            ▲                         ▲              │
│            │                         │              │
│  ┌────────────────────────────────────────────────┐ │
│  │ 数据库 API 层 (CRUDBase/QueryBuilder)           │ │
│  │ + 优化层(3级缓存/预加载/批调度)                 │ │
│  └────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────┘
```
说明：
- 记忆构建在对话中快速入库（临时池），整理任务由统一调度器按时间或事件触发批量执行。
- 所有持久化与读写通过数据库 API 层进行，结合优化层的三级缓存与批量调度以保障性能。
- 与事件系统对齐：在 `ON_MESSAGE_RECEIVED` 等事件中触发检索/构建；在 `ON_START` 启动时注册整理任务。
```
...
#### 2.2.3 存储层
为与项目的数据库重构保持一致，存储层需遵循 MoFox 六层数据库架构与优化策略：
- **API 层 (CRUDBase/QueryBuilder)**：统一的读写入口，避免直接使用 `Session`；对节点、边、记忆等模型进行增删改查与链式查询。
- **优化层 (3级缓存/预加载/批量调度)**：
  - L1 内存缓存：热点节点/记忆的快速访问；
  - L2 SQLite 缓存：中短期数据缓冲与轻量聚合；
  - L3 预加载器：根据访问模式提前加载关联子图；
  - AdaptiveBatchScheduler：批量插入/更新，避免循环逐条写入。
- **Vector Store（可选组件）**：用于节点/主题/客体的语义向量检索（如 ChromaDB 或内置向量索引封装），需与数据库模型的 `embedding` 字段保持一致，支持同步/异步刷新。
- **Graph Index（运行时索引）**：以邻接表或轻量图结构维护运行时关系视图，支持 BFS/关系过滤等操作；索引与持久化模型解耦，通过事件或调度器周期性刷新。
- **Persistence（持久化）**：以 SQLAlchemy 2.0 模型统一定义并存储到主库（SQLite/PostgreSQL）；不再使用散 JSON 文件直接持久化，若需导出采用导出工具。

注意：所有批量操作应优先通过 `AdaptiveBatchScheduler` 执行；查询偏好 `QueryBuilder` 的链式过滤与分页，尽量减少频繁小查询。
...
class MemoryConsolidator:
  ...
  ### 12.5 统一调度器整合（Unified Scheduler）

  与主系统的统一调度器集成，采用“定时 + 事件”双触发方式：
  ...
  async def register_consolidation_tasks():
    ...
    await unified_scheduler.create_schedule(
      callback=run_memory_consolidation,           # 绑定 MemoryConsolidator.run_consolidation
      trigger_type=TriggerType.TIME,
      trigger_config={"delay_seconds": 24 * 3600},
      is_recurring=True,
      task_name="daily_memory_consolidation"
    )

    # 事件触发：当累计达到阈值条“staged”记忆时触发一次
    await unified_scheduler.create_schedule(
  ...
  建议：
  - 初始周期可设为 6-24 小时；视数据增长速度与资源情况进行动态调整。
  - 结合优化层的预加载器，在整理前预取热点主体与最近活跃记忆子图。

  ---

  ### 12.6 事件系统与权限对接

  事件系统（`event_manager`）用于触发构建/检索逻辑：
  ...
  权限系统：
  - 在工具调用与管理接口中加入节点权限检查（如 `plugin.memory_graph.admin`）。
  - Master 用户配置见 `bot_config.toml` 的 `[permission.master_users]`；敏感操作（批量清理、导出）需 Master 或管理员权限。
  ...
  与项目的双轨记忆架构对齐：
  - **Memory Graph**：持久化的图记忆（人物、事件、关系），适合长期语义检索与推理。
  - **Chat Memory**：会话上下文记忆，短期、轻量，提升对话连续性。

  协同策略：
  - 在检索阶段，先以 Chat Memory 提供语境缩窄，再用 Memory Graph 做深度召回与多跳扩展。
  - 引入兴趣值系统的主题聚类与时间衰减，动态调整检索评分中的 `importance` 与 `time_decay` 权重（AFC 策略）。
  - 对高兴趣主题的相关记忆，优先进入 L1/L2 缓存，形成“热点子图”。

  参考：
  - docs/memory_graph_guide.md（整体说明）
  - docs/unified_scheduler_guide.md（调度器使用）
  - docs/three_tier_memory_user_guide.md（双轨记忆/三级缓存）

  ---

  ## 13. 安全、隐私与合规

  - **隐私标记**：对敏感字段（身份、联系方式、位置）的节点与边添加隐私标签；默认检索不返回隐私内容，需权限显式开启。
  - **数据最小化**：仅在必要时存储原文或引述；优先结构化信息，减少敏感文本暴露。
  - **访问控制**：通过权限节点限制批量导出、清理、合并等高风险操作；Master 用户方可执行紧急维护命令。
  - **审计日志**：对记忆的新增/修改/删除与批处理记录结构化日志，定位到主体与任务编号，便于复盘与溯源。
  - **合规参考**：遵循项目的 `PRIVACY.md` 与公开文档要求，避免在默认配置中开启广域数据采集。
  ...
  ### v0.2 (2025-12-22)
  - 对接统一调度器，新增定时/事件触发的记忆整理流程与示例代码。
  - 存储层说明更新：采用数据库 API 层 + 优化层（3级缓存/预加载/批量调度），不直接使用 `Session`。
  - 增补事件系统与权限系统对接，明确敏感操作的访问控制与审计要求。
  - 引入双轨记忆与兴趣系统的协同策略，建议在检索评分中融合兴趣权重与时间衰减。
  - 添加安全与隐私章节，明确隐私标记、访问控制与日志审计。
